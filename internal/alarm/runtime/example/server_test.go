...
func TestRunAlarmReturnsUnsupportedWhenNoAlarmsLoaded(t *testing.T) {
	...
	resp, err := srv.RunAlarm(context.Background(), &runtimev1.RunAlarmRequest{
		AlarmId: "alarm-1",
	})
	...
	assert.Equal(t, runtimev1.RuntimeErrorCode_RUNTIME_ERROR_CODE_UNSUPPORTED, resp.GetError().GetCode())
...
func TestRunAlarmRejectsEmptyAlarmID(t *testing.T) {
	...
	_, err := srv.RunAlarm(context.Background(), &runtimev1.RunAlarmRequest{})
	require.Error(t, err)
}
